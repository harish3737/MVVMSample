    ...
    func getPlaceAddress(from location : LocationCoordinate, on completion : @escaping ((LocationDetail)->())){
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            if let places = data?.getDecodedObject(from: Place.self), let address = places.results?.first?.formatted_address, let lattitude = places.results?.first?.geometry?.location?.lat, let longitude = places.results?.first?.geometry?.location?.lng {
                ...
                print("places>>>>","\(places.results?.first?.formatted_address ?? "")")
                print("places1>>>>",places.results?.first ?? "")
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            ...
            if let updatedLocation = updatedLocation {
                geoCoder.reverseGeocodeLocation(updatedLocation, completionHandler: { placemarks, error in
                    if error == nil {
                        let placemark = placemarks?[0]
                        print("Country code : \(placemark?.country ?? "")")
                        print("Country code : \(placemark?.isoCountryCode ?? "")")
                        /// currectCountryISO your current location country ISO code.
                        currentCountryISO = (placemark?.isoCountryCode) ?? ""
                    }
                })
            }
