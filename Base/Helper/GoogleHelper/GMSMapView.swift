...
extension GMSMapView {
    ...
    private func getGoogleResponse(between source: CLLocationCoordinate2D, to destination: CLLocationCoordinate2D, completion : @escaping((MapPath)->Void)) {
        ...
        print("url>>>","https://maps.googleapis.com/maps/api/directions/json?origin=\(source.latitude),\(source.longitude)&destination=\(destination.latitude),\(destination.longitude)&sensor=false&mode=driving&key=\(googleMapKey)")
    ...
    private func drawPath(with points : String){
        ...
        DispatchQueue.main.async {
            ...
            polyline.strokeColor = .primaryColor
            polyline.map = self
